...
export async function GET() {
  ...
  if (!session || session.user?.role !== 'admin') {
    return new NextResponse(JSON.stringify({ message: 'Acceso no autorizado' }), { status: 401 });
  }
  ...
  try {
    ...
    return new NextResponse(JSON.stringify({ message: 'Error interno del servidor al obtener usuarios' }), { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!id || !role || (role !== 'user' && role !== 'admin')) {
      return new NextResponse(JSON.stringify({ message: 'ID de usuario o rol no válidos' }), { status: 400 });
    }
    ...
    if (session.user?.id === id && role !== 'admin') {
      ...
      return new NextResponse(JSON.stringify({ message: 'No puedes cambiar tu propio rol de administrador directamente.' }), { status: 403 });
    }
    ...
    return new NextResponse(JSON.stringify({ message: 'Error interno del servidor al actualizar usuario' }), { status: 500 });
  }
}